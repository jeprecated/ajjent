...
- **Tidying** automatically selects only normally **Closable Disposable Workspaces**, excluding Main and Current. Keep and missing registrations remain manually selectable. Automatic preselection is computed greedily in row order so it never contains rows that only other preselected rows protect; those start unchecked (and are named, under `--yes`). Contradictory manually selected batches block submission with a visible `Enter blocked` reason rather than silently filtering targets. Zero checked rows and cancellation do not delete, abandon, or clean leftovers.
