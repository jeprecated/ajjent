...
To branch independently of the Current Workspace's mutable head, add optional `child.baseCommit` with a full 40-character lowercase hexadecimal commit ID. For example, Summon should capture `@` for `target.expectedHeadCommit` and its sole `@-` for `child.baseCommit`. Ajj still checks the Current Workspace head for drift, but creates/reconciles the child against the explicit base. Omission preserves creation from the asserted head; human create is unchanged.
...
Workspace conflict status checks **that Workspace's own mutable ancestor history, including its head**. A conflicted sibling sharing a mutable merge does not make a clean Workspace conflicted. Genuine conflicted mutable heads/ancestors still block normal Closing even when their work is represented elsewhere; unique nonconflicted work remains protected independently. See the [synthetic agent-topology regression evidence](docs/workspace-conflict-scope-evidence.md).

Close confirmation explains representation by surviving Workspaces, independently of the Main-relative status. An empty working-copy head can still have unstacked ancestors. Described empty commits (including checkpoints and merges) remain relevant history; only empty undescribed cursors are disposable.

Close and Tidy collect all confirmations, including outside-layout directory deletion, before abandoning changes. Directory cleanup retries permission failures by granting owner write/search permissions to directories inside the selected Workspace; it never follows symlinks, changes file permissions, or uses sudo. Failures such as an unwritable parent directory remain errors and retain the jj registration. Cleanup is not atomic: on failure Ajj stops and reports completed abandonments, closed Handles, the failed target, and unattempted targets. Earlier mutations are not rolled back.

Before `ajj close` / `ajj tidy` selection, Ajj snapshots present candidate Workspaces with `jj status` and reloads graph safety, so unsnapshotted tracked edits and newly tracked files cannot inherit an old empty cursor's safety. Snapshot failures and stale working copies stop the command; Ajj does not automatically recover stale candidates. `ajj tidy` scopes a stale working copy to that Workspace: it is shown disabled with its `jj -R <path> workspace update-stale` hint (a warning under `--yes`) and never closed, while other Workspaces can still be tidied. A cancelled review may still have recorded working-copy edits, but does not authorize abandonment or deletion. After confirmation, Ajj snapshots the selected candidates again and aborts if the reviewed repository operation changed, including in force mode: rerun and review the new state. This is not atomic protection against concurrent writers. Ignored files and files JJ declines to track are outside the graph-safety proof; preserve those separately before Closing. Post-Stack Closing uses the same guard: only after Stack computation finishes, it snapshots the proposed closing targets and rechecks their complete-batch safety before the first close prompt. Both that prompt and any later outside-layout consent are bound to the reviewed state.
...
**Tidy preview:** press **v** for the highlighted Workspace's bounded read-only evidence view; press **v** again to return. **↑/↓** still moves between Workspaces, **PgUp/PgDn** scrolls detail, and **Space / p / f / Enter / q / Esc** retain their selection, policy, force, submit, and cancel meanings. Keep/Disposable intent, Main-relative status, and graph evidence remain separate.

The preview shows relevant mutable ancestor change/commit IDs and descriptions (including ancestors beneath an empty cursor), unique revision detail against the currently surviving set, and **Main → highlighted Workspace** changed-files/patch comparisons. Selecting a protector changes the unique detail. **A Main diff is not unique-work or ancestry proof.** Missing Workspace previews show registered history, not disk content; conflicts are explicitly identified. There is no inferred origin/parent grouping and no arbitrary-history browser.

Safety evidence and preview share one pinned JJ operation (`--ignore-working-copy --at-op=<full-id>`). Previewing never snapshots, edits, invokes an external pager/diff formatter, or changes closing eligibility. Reads run asynchronously, obsolete requests are cancelled, and late results cannot replace a new highlight/selection. Errors and truncation are visible; repository-controlled terminal control characters are replaced. Limits are 30 log/detail rows per section, 80 changed-file rows, 120 patch rows, 64 KiB per preview command, and an eight-second request deadline. Long displayed lines are clipped with an ellipsis. Initial complete graph membership has a 15-second preparation deadline and 2 MiB per-Workspace read cap; exceeding it fails closed rather than treating partial evidence as safe.

Tidy also binds the reviewed cleanup policy and Workspace identity independently of JJ's operation ID. Revoking Disposable (`ajj keep`) or changing identity while a selector/confirmation is open aborts at the final guard with rerun/review guidance, including Forced Tidying and outside-layout consent. The TUI's own successful `p` action updates that row's accepted policy before submission; stable explicitly selected Keep rows remain closable. This does not change explicit Close. Policy validation is a final preflight check, not a lock against later concurrent writes.
