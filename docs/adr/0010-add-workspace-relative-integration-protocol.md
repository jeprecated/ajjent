...
Before effects, v1 additionally requires the target head to be empty, undescribed, and nonconflicted and every payload head to be exact and nonconflicted. A non-empty undescribed payload head is also rejected as in-progress working-copy state. Callers must materialize filesystem changes before capturing request heads; Ajj deliberately does not snapshot or update stale sibling working copies after those exact heads are bound because doing so could rewrite the request inputs. Preserving in-progress target or payload work is outside v1.
...
```json
{
  ...
  "evidenceDigest": "sha256:4607a7a7eda2f1e381eec5ac797c93f800518edeaf0e36af12972b00401f0d96"
}
```
...
`beforeHeadCommit` is the asserted Current Workspace cursor. `integratedTipCommit` is the final adopted payload tip. `afterHeadCommit` is the target's resulting fresh empty working-copy cursor. These values must not be conflated.
...
`single` and `provider-default` call the same Stack rebase, clean detached insertion probe, conflict fallback, target-finalization, and input-advance functions as ordinary Stack without recursively invoking the human CLI. Machine execution wraps the shared mutation boundary with journal intents and proof; it does not maintain a second graph algorithm. Ajj materializes each payload's exact non-empty changes and frontier relative to the Current Workspace before effects. Stack's configured auto/linear/merge shape, branch/revision mode, and prefer-clean behavior remain authoritative. The configured Main Workspace remains unchanged unless it is Current.

After the target reaches every exact payload change and target finalization leaves one integrated tip, Ajj records `target-advanced` and the full Jujutsu commit-point operation id. It then uses ordinary Stack's input-advance behavior as recoverable housekeeping. Receipts require a one-for-one ordered mapping from every prepared input change id and commit to exactly one landed commit proved under target ancestry; merge-preserved and immutable payloads therefore have equal input and landed commit ids, while clean insertion records the rewritten commit id. Empty or unrelated success mappings are invalid journal state.
